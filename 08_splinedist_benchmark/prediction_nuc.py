...
# -*- coding: utf-8 -*-
"""
prediction_nuc.py — NUCLEI segmentation with SplineDist (DAPI only, pretrained).
Minimal script: load SampleDataset, run SplineDist pretrained model, save masks.
"""
...
def run_splinedist_single(img2d: np.ndarray) -> np.ndarray:
    ...
    labels, _ = MODEL.predict_instances(x)  # 可按需加 n_tiles=(2,2) 处理大图
...
def main():
    ...
    for s in ds:
        try:
            ...
            mask = run_splinedist_single(s.nuc_chan)

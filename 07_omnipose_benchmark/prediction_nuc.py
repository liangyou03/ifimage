...
OUTPUT_DIR = Path("nuclei_prediction_omnipose")
...
# init once — explicit nclasses avoids NoneType arithmetic in some branches
APP = CellposeModel(
    gpu=USE_GPU,
...
def _eval_omni(x: np.ndarray):
    ...
    # Support 3- or 4-tuple; some branches return dict, just in case
...
def main():
    ...
    for s in ds:
        try:
            ...
            mask = run_omni_single(s.nuc_chan)
            np.save(outp, mask)

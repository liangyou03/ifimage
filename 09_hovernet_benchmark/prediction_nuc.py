...
OUT_DIR    = ROOT / "nuclei_prediction"
...
def to_uint8_rgb(gray: np.ndarray) -> np.ndarray:
    if gray.dtype != np.uint8:
        ...
        lo, hi = np.percentile(g, (0.5, 99)); hi = max(hi, lo + 1e-6)
...
def collect_inst_maps():
    ...
    for m in sorted(mat_dir.glob("*.mat")):
        ...
        np.save(OUT_DIR / f"{m.stem}.npy", inst)
        print(f"[OK] {m.stem} -> {m.stem}.npy (labels={int(inst.max())})")
